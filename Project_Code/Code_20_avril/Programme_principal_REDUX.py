#Programme_principal_REDUX.py

#  @file   Programme_principal_REDUX.py
...
#    def pidTemp(self):        
#        if(obj_mesure.tempDS != -1):
#            control = pid_HeatMat(obj_mesure.tempDS)

#            print(control)
#            controlcent = control*100/255
#            print(controlcent)
#            soft_pwm.ChangeDutyCycle(controlcent)
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        obj_control.cont_temp = incomingMQTT["TEMP"]   #Constante Température changé
        obj_control.cont_hum = incomingMQTT["HUM"]   #Constante d'humidité changé
        print(obj_control.cont_temp)
        print(obj_control.cont_hum)
        obj_control.pid_HeatMat.setpoint = obj_control.cont_temp
    except Exception as err:
        print(err)

obj_mesure = classes.c_Mesure()
obj_control = classes.c_Control()
...
if __name__ == '__main__':
    ...
    #humid_Sol = -1
    '''try:
        ser = serial.Serial('/dev/serial0', 9600, timeout=2)  #Initialise la connection avec le ARDUINO NANO
    except Exception as err:
        print(str(err))'''
    ...
    while True:
        ...
        if time()-SensorPrevSec > SensorInterval:  #Si + de deux secondes
            ...
            #obj_mesure.updateValue(tup_Read)
            obj_mesure.updateValue()

            print("\nValue from Arduino")
            print("CO2 (ppm) : " + str(obj_mesure.co2))
            print("TVOC : " + str(obj_mesure.tvoc))
            print("Temp LED (°C) : " + str(obj_mesure.tempDS))
            print("Humid Sol (%RH) : " + str(obj_mesure.humSEN))
            print("Temp Sol (°C) : " + str(obj_mesure.tempSEN)+"\n")
            
            obj_control.pidTemp(obj_mesure.tempDS)
                
            #Section d'envoie MQTT pour le MainGUI.py
            try:
                json_data = json.loads('{"DS": {"temp": '+str(obj_mesure.tempDS)+'}, "SHT": {"temp": '+str(obj_mesure.tempSEN)+', "hum": '+str(obj_mesure.humSEN)+'}, "CSS811": {"CO2": '+str(obj_mesure.co2)+', "TVOC": '+str(obj_mesure.tvoc)+'}}')
                topic = "/test"
        ...
        if time()-ThingSpeakPrevSec > ThingSpeakInterval:     #Si fait plus de une minute depuis la dernière transmission sur ThingSpeak.
            ...
            save_To_Thingspeak(obj_mesure.co2,obj_mesure.tempDS,obj_mesure.tempSEN,obj_mesure.humSEN, obj_mesure.tvoc)
